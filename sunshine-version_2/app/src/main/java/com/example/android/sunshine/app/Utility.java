...
public class Utility {
    ...
    static public boolean isNetworkAvailable(Context context){
        //ConnectivityManager를 통해 인터넷 연결 상태를 체크한다.
        ConnectivityManager connectivityManager = (ConnectivityManager)context.getSystemService(Context.CONNECTIVITY_SERVICE);
        //와이파이건 모바일이건 연결이된상태를 값을 받는다. getTypeName으로 mobile인지 wifi인지확인가능.
        NetworkInfo networkInfo = connectivityManager.getActiveNetworkInfo();
        //networkInfo가 Null이 아니면 무언가는 연결이 된것이고
        //isconnectedOrconnecting을 통해 상태에 상관없이 네트웤이 제대로 연결되었는지 확인한다.
        return networkInfo != null && networkInfo.isConnectedOrConnecting();
    }
    @SuppressWarnings("ResourceType")
    static public @SunshineSyncAdapter.LocationStatus int getLocationStatus(Context context){
        SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(context);
        return sharedPreferences.getInt(context.getString(R.string.pref_location_status_key), SunshineSyncAdapter.LOCATION_STATUS_UNKOWN);
    }
    static public void resetLocationStatus(Context context){
        SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(context);
        SharedPreferences.Editor editor = sharedPreferences.edit();
        editor.putInt(context.getString(R.string.pref_location_status_key), SunshineSyncAdapter.LOCATION_STATUS_UNKOWN);
        editor.apply();
    }
}